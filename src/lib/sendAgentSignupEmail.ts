...
/**
 * Resolves the email of the client's referral agent/broker.
 *
 * Two ways a client can arrive with an agent attached:
 ...
 * flips is_deleted, so we filter it explicitly). Returns null when the lead has
 * no agent on file — a normal, non-error case (direct sign-ups have no referrer).
 */
...
export async function sendAgentSignupEmail(
  ...
  console.log(
    `[Agent Signup Email] Sent to ${agentEmail} (source=${opts.source ?? "unknown"}), subject="${subject}", id=${sendResult?.id}`,
  );
